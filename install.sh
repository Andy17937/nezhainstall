...
pre_check() {
    ...
    if [ -n "$CUSTOM_MIRROR" ]; then
        ...
        if [ -z "$CN" ]; then
            GITHUB_RAW_URL="ghproxy.shota.sg/nezhahq/scripts/v0"
            GITHUB_URL="github.shota.sg"
            Get_Docker_URL="get.docker.com"
            Get_Docker_Argu=" "
...
modify_dashboard_config() {
    ...
    echo "关于 GitHub Oauth2 应用：在 https://github.shota.sg/settings/developers 创建，无需审核，Callback 填 http(s)://域名或IP/oauth2/callback"
...
restart_and_update_standalone() {
    # _version=$(curl -m 10 -sL "https://api.github.shota.sg/repos/naiba/nezha/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
    # if [ -z "$_version" ]; then
    ...
    if [ -z "$_version" ]; then
        err "获取 Dashboard 版本号失败，请检查本机能否链接 https://api.github.shota.sg/repos/naiba/nezha/releases/latest"
        return 1
    else
        echo "当前最新版本为： ${_version}"
    fi
...
show_menu() {
    ...
    --- https://github.shota.sg/naiba/nezha ---
